...
namespace Alibris_OrderConfirmation
{
    public class Alibris_OrderConfirm : ModuleBase
    {
        ...
        public Alibris_OrderConfirm(ILogger logger, ISettings settings) : base(logger, settings)
        {
        ...
        private bool GetAlibrisDetails()
        {
            bool result = false;
        ...
        private string CreateConfirmFile(DataTable updateRecord)
        {
            ...
            string FileName = Path.Combine(assemblyFile, DateTime.Now.ToString("yyyyMMddhhmmss") + ".tab");
            StreamWriter sw = new StreamWriter(FileName);
            sw.WriteLine("Order/PN Number\tStatus\tDelivery Confirmation");
            foreach (DataRow dr in updateRecord.Rows)
            {
                sw.WriteLine(dr["OrderID"] + "\tSH\t" + Convert.ToString(dr["LogisticName"]) + Convert.ToString(dr["TrackingNumber"]));
            }
            //UPS__ , USPS_ , DHL__ , FEDEX, UPSMI, NEWGI, DHLGM, ONTRA, and CPOST
            sw.Close();
            return FileName;
        }
